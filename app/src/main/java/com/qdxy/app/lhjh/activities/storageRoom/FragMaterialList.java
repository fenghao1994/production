...
public class FragMaterialList extends TempFragment {
    private final String TAG = "FragMaterialList";
    ...
    protected void setListeners(View view, @Nullable Bundle savedInstanceState) {
        EventBus.getDefault().register(FragMaterialList.this);
        mPrePullable = new TempPullablePreDefault<RespStorageList>(new TempPullableViewI<RespStorageList>() {
            ...
            public Call<RespStorageList> createObservable(int queryPage, int querysize, int currentPage) {
                ...
                return TempRemotAPIConnecter.INSTANCE.createRemoteApi(TempAPI.class).orderList_auxiliary(tempType,mIsHandle,queryPage);
            }
        };
    ...
    protected void OnViewClicked(View v) {
        switch (v.getId()) {
            case R.id.fab:
                if (mType==CommonType.MATERIAL_SQ){
                    Debug.info(TAG,"辅料领用点击");
                    Intent tempIntent = new Intent(getActivity(), ActMaterialCreate.class);
...
//        Debug.info(TAG, "event 接受到数据" + event);
    ...
    public void onDestroy() {
        EventBus.getDefault().unregister(FragMaterialList.this);
        super.onDestroy();
    }
    @Override
