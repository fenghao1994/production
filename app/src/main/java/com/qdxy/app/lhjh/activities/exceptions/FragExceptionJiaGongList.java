...
public class FragExceptionJiaGongList extends TempFragment {
    ...
    protected void setListeners(View view, @Nullable Bundle savedInstanceState) {
        EventBus.getDefault().register(FragExceptionJiaGongList.this);
        isJudge = getArguments().getBoolean("isJudge");
        mTempRVCommonAdapter = new TempRVCommonAdapter<RespExpList.DataBean.DatasBean>(getActivity(), R.layout.item_exception_jiagong_layout) {
            @Override
            public void bindItemValues(TempRVHolder holder, RespExpList.DataBean.DatasBean s) {
                holder.setText(R.id.item_exception_jiagong_creationTime,isJudge?s.getHandleTime():s.getCreationTime());
                holder.setText(R.id.item_exception_jiagong_index0,"发送人："+s.getSender());
                holder.setText(R.id.item_exception_jiagong_index1,"工序："+s.getProcedureName());
                holder.setText(R.id.item_exception_jiagong_index2,"产品号："+s.getProductCode());
                holder.setText(R.id.item_exception_jiagong_index4,"生产线："+s.getProductionLineName());
//                holder.setText(R.id.item_exception_jiagong_index3,"编号"+s.getCreationTime());
            }
        };
        ...
        mTempPullablePresenterI = new TempPullablePreDefault<RespExpList>(mTempPullableViewI) {
            ...
            public Call<RespExpList> createObservable(int queryPage, int querysize, int currentPage) {
                return TempRemotAPIConnecter.INSTANCE.createRemoteApi(TempAPI.class).machiningPartProblemList(isJudge,queryPage);
            }
        };
    }
    ...
    private void initReceiverview(TempRefreshRecyclerView recyclerview, TempRVCommonAdapter adapter) {
        ...
        adapter.setOnItemClickListener(new OnItemClickListener<RespExpList.DataBean.DatasBean>() {
            ...
            public void onItemClick(ViewGroup parent, View view, RespExpList.DataBean.DatasBean o, int position) {
                Intent dajuDealIntent  = new Intent(getActivity(),ActJiaGongDeal.class);
...
//        recyclerview.addItemDecoration(new TempRVDividerDecoration(Color.parseColor("#EEEFF3"), TempDensityUtil.dip2px(ActExceptionJiaGongList.this, 12.0f)));
    }
...
//        Debug.info(TAG, "event 接受到数据" + event);
    ...
    public void onDestroy() {
        EventBus.getDefault().unregister(FragExceptionJiaGongList.this);
        super.onDestroy();
    }
}
