...
public class FragExceptionEquList extends TempFragment {
   ...
   /* @Override
    ...
    protected void findViews() {
        ...
        initToolbar(toolbarTop, "设备异常列表");
    }*/

    @Override
    protected void setListeners(View view, @Nullable Bundle savedInstanceState) {
        EventBus.getDefault().register(FragExceptionEquList.this);
        isJudge = getArguments().getBoolean("isJudge");
        mTempRVCommonAdapter = new TempRVCommonAdapter<RespExpList.DataBean.DatasBean>(getActivity(), R.layout.item_exception_equ_layout) {
            @Override
            public void bindItemValues(TempRVHolder holder, RespExpList.DataBean.DatasBean s) {
                holder.setText(R.id.item_exception_content_index0,"发送人："+s.getSender());
                holder.setText(R.id.item_exception_equ_creationTime,isJudge?s.getHandleTime():s.getCreationTime());
                holder.setText(R.id.item_exception_content_index1,"工序："+s.getProcedureName());
                holder.setText(R.id.item_exception_content_index2,"设备名："+s.getDeviceCode());
                holder.setText(R.id.item_exception_content_index3,"生产线："+s.getProductionLineName());
            }
        };
        ...
        mTempPullablePresenterI = new TempPullablePreDefault<RespExpList>(mTempPullableViewI) {
            ...
            public Call<RespExpList> createObservable(int queryPage, int querysize, int currentPage) {
                return TempRemotAPIConnecter.INSTANCE.createRemoteApi(TempAPI.class).deviceProblemList(isJudge,queryPage);
            }
        };
    }
    ...
    private void initReceiverview(TempRefreshRecyclerView recyclerview, TempRVCommonAdapter adapter) {
        ...
        adapter.setOnItemClickListener(new OnItemClickListener<RespExpList.DataBean.DatasBean>() {
            ...
            public void onItemClick(ViewGroup parent, View view, RespExpList.DataBean.DatasBean o, int position) {
                Intent dajuDealIntent  = new Intent(getActivity(),ActEquDeal.class);
...
//        Debug.info(TAG, "event 接受到数据" + event);
    ...
    public void onDestroy() {
        EventBus.getDefault().unregister(FragExceptionEquList.this);
        super.onDestroy();
    }
}
