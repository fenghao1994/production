...
//
    ...
    public final static String PERMISSION_TABLE_NAME = "permission_info";//保存权限信息的xml名字
...
//    public final static String LOGIN_SEX;//积分     1 1
    public final static String LOGIN_TOKEN;//TOKEN  1
...
//    public final static String LOGIN_GRADESORT;//vip
    ...
    public final static String LOGIN_MEMBER_TYPE;
    public final static String JPUSH_TABLE;
...
//    public final static String LOGIN_LOGTYPE; //用户登录类型 （会员，商户，管理员，业主，VIP...）
    public final static String LOGIN_USER_ID; //用户ID
    public final static String LOGIN_PHONE;
    public final static String WORK_STATUS;

    //    public final static String LOGIN_DELETE; //是否启用 （1 - 启用 ，0 - 停用）
//
////
    static {
        JPUSH_TABLE="jpush_table";
...
//        LOGIN_LOGTYPE = "logintype";
        LOGIN_MEMBER_TYPE="member_type";
        LOGIN_USER_ID = "museId";
...
//
     ...
     * @param permission
     */
    public static void sf_savePermission(List<String> permission) {
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(PERMISSION_TABLE_NAME, Context.MODE_PRIVATE);

            SharedPreferences.Editor editor = sf.edit();
            editor.clear();
            for (int i=0;i<permission.size();i++){
                editor.putBoolean(permission.get(i), true);
            }
            editor.commit();
    ...
    public static void clearPermissionTable(){
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(PERMISSION_TABLE_NAME, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sf.edit();
            editor.clear();
     ...
     * @return
     */
    public static boolean getPermissionStatus(String name) {
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(PERMISSION_TABLE_NAME, Context.MODE_PRIVATE);
//            sf.contains(name)
            return  sf.contains(name);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
     ...
     * @param status
     */
    public static void sf_saveJPust_status(String alias,boolean status) {
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(JPUSH_TABLE, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sf.edit();
            editor.clear();
            editor.putBoolean(alias, status);
            editor.commit();
    ...
    public static void clearJPushTable(){
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(JPUSH_TABLE, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sf.edit();
            editor.clear();
     ...
     * @return
     */
    public static boolean getJPust_status(String name) {
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(JPUSH_TABLE, Context.MODE_PRIVATE);
  ...
  /*  *//**
     ...
     * @param alias
     *//*
    public static void sf_saveJPust_alias(String alias) {
        try {
    ...
    *//**
     ...
     * @return
     *//*
    public static String getJPust_alias() {
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOCATION_INFO, Context.MODE_PRIVATE);
            return sf.getString(JPUSH_ALIAS,"");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
...
//        }
    ...
    // 保存用户登录名
    public static void sf_saveUserName(String userName) {
//        try {
//
//            snappyDB.putBoolean(LOGIN_REMEMBER_STATE, state);
//            Debug.debug(TAG, "保存LOGIN_REMEMBER_STATE=" + snappyDB.getBoolean(LOGIN_REMEMBER_STATE));
...
//        }
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOGIN_TABLE_NAME, Context.MODE_PRIVATE);
            Debug.debug(TAG, "LOGIN_MEMBER_TYPE=" + sf.getInt(LOGIN_MEMBER_TYPE, 1));
            return sf.getInt(LOGIN_MEMBER_TYPE, 1);
        } catch (Exception e) {
    ...
    public static  int sf_getWorkStatus(){
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOGIN_TABLE_NAME, Context.MODE_PRIVATE);
            Debug.debug(TAG, "LOGIN_MEMBER_TYPE=" + sf.getInt(WORK_STATUS, 1));
            return sf.getInt(WORK_STATUS, 1);
        } catch (Exception e) {

        }
        return 1;
    }
    public static void sf_saveWorkStatus(int workStatus) {
        SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOGIN_TABLE_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sf.edit();
        editor.putInt(WORK_STATUS, workStatus);
        editor.commit();
    }
    //    *保存用户名和密码
//     * @param username
...
//        editor.putString(LOGIN_LOGTYPE, LoginInfo.getResult().getSuseType()+"");//账户登录类型
            editor.putString(LOGIN_USER_ID, LoginInfo.getResult().getMuseId() + "");//用户ID
//        editor.putString(LOGIN_SCORE,LoginInfo.getData().getScore());
            editor.putString(LOGIN_TOKEN, LoginInfo.getResult().getMuseOnlineTag());
            ...
            editor.putInt(WORK_STATUS, LoginInfo.getResult().getWorksSatus());
//            editor.putInt(LOGIN_MEMBER_TYPE, LoginInfo.getResult().getMuseType());
//        editor.putString(LOGIN_HEAD,LoginInfo.getResult().getSuseAvatarImageUrl());
...
//        try {
//            Debug.debug(TAG, "获取LOGIN_USERID=" + snappyDB.get(LOGIN_SUSEID));
...
//        }
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOGIN_TABLE_NAME, Context.MODE_PRIVATE);
            Debug.debug(TAG, "LOGIN_userID=" + sf.getString(LOGIN_USER_ID, ""));
            return sf.getString(TempLoginConfig.LOGIN_USER_ID, "");
...
//     * @return
//
    public static String sf_getToken() {
//        try {
...
//        }
        ...
        try {
            SharedPreferences sf = TempApplication.getInstance().getApplicationContext().getSharedPreferences(LOGIN_TABLE_NAME, Context.MODE_PRIVATE);
//            Debug.debug(TAG, "get LOGIN_TOKEN=" + sf.getString(LOGIN_TOKEN, ""));
            return sf.getString(LOGIN_TOKEN, "");
        } catch (Exception e) {
