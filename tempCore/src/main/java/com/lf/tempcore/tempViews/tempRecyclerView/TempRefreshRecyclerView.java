...
public class TempRefreshRecyclerView extends FrameLayout {
    ...
    public TempRefreshRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (isInEditMode()){
            return;
        }
    ...
    public TempRefreshRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void initAttrs(AttributeSet attrs) {
       TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.tempRefreshRecyclerView);
         try {
            mClipToPadding = a.getBoolean(R.styleable.tempRefreshRecyclerView_recyclerClipToPadding, false);
            mPadding = (int) a.getDimension(R.styleable.tempRefreshRecyclerView_recyclerPadding, -1.0f);
            mPaddingTop = (int) a.getDimension(R.styleable.tempRefreshRecyclerView_recyclerPaddingTop, 0.0f);
            mPaddingBottom = (int) a.getDimension(R.styleable.tempRefreshRecyclerView_recyclerPaddingBottom, 0.0f);
            mPaddingLeft = (int) a.getDimension(R.styleable.tempRefreshRecyclerView_recyclerPaddingLeft, 0.0f);
            mPaddingRight = (int) a.getDimension(R.styleable.tempRefreshRecyclerView_recyclerPaddingRight, 0.0f);
            mScrollbarStyle = a.getInteger(R.styleable.tempRefreshRecyclerView_scrollbarStyle, -1);

            mEmptyId = a.getResourceId(R.styleable.tempRefreshRecyclerView_layout_empty, 0);
            mProgressId = a.getResourceId(R.styleable.tempRefreshRecyclerView_layout_progress, 0);
            mErrorId = a.getResourceId(R.styleable.tempRefreshRecyclerView_layout_error, 0);
        } finally {
            a.recycle();
        }
    }
    ...
    private void initView() {
        ...
        //生成主View
       View v = LayoutInflater.from(getContext()).inflate(R.layout.temp_layout_progress_recyclerview, this);
         mPtrLayout = (TempSwipeRefreshLayout) v.findViewById(R.id.ptr_layout);
...
//        setProgressView(R.layout.temp_rv_progressbar);
    ...
    public static class TempRVDataObserver extends RecyclerView.AdapterDataObserver {
        ...
        public TempRVDataObserver(TempRefreshRecyclerView recyclerView) {
        ...
        private void update() {
            log("update");
            int count;
            if (recyclerView.getAdapter() instanceof TempRVCommonAdapter) {
                count = ((TempRVCommonAdapter) recyclerView.getAdapter()).getCount();
            } else {
